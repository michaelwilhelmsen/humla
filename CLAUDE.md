...
- **Chat over your notes** — an agentic retrieval loop (`chat/`), surfaced as the **Chat tab of the Note right panel**. The assistant searches and reads notes with three tools (`search_notes` over FTS5 keyword + semantic embeddings, `get_note`, `list_notes`), streams its answer, and **cites the notes it drew from** as chips that navigate to the source. A per-conversation **breadth** (`note` | `folder` | `all`, persisted on the conversation row) is a live filter on retrieval. Multiple conversations per note, with history. Runs on a chat provider configured **separately** from transcription and summary (Settings → Chat; OpenAI or Ollama). In a workspace, turns route to humla-cloud and retrieval happens server-side. ⚠️ Chat currently has **no route of its own** — it's reachable only inside a note; a library-wide `/chat` surface is in flight (issues #80–#82).
...
- **Design system** (redesigned in v0.30.0 — the old "Nothing" aesthetic with Space Grotesk + Space Mono is **gone**, don't reintroduce it):
  ...
  - **Utilities** (`src/styles/globals.css`): `.nd-btn` / `.nd-btn-primary` / `.nd-btn-icon` / `.nd-btn-icon-sm` (all pill/circle radius), `.nd-chip`, `.nd-ctl`, `.nd-meta`, `.nd-label`, `.nd-bare`, `.nd-recpill`, `.nd-speaker-pill`, `.nd-speaker-dot`, `.nd-word` / `.nd-word-active`. **`.nd-action`, `.nd-prop-*`, `.nd-select` and `.rec-bar` were deleted** — don't reference them.
